...
public struct ProgressModel: Model {
    var progress = MatchaBubbles.Progress()
    var downloadProgress = MatchaBubbles.Progress()
    var animationProgress = MatchaBubbles.Progress()
    ...
    public func view() -> String {
        ...
        let downloadStatus = isRunning && download.percent < 1.0
            ? " Downloading..."
            : download.percent >= 1.0 ? " Complete!" : " Ready"
        ...
        let animStatus = isRunning && animation.percent < 1.0
            ? " Frame \(frames)/60"
            : animation.percent >= 1.0 ? " Done!" : " Ready"
