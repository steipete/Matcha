...
//  Spinner Example
//
//  Demonstrates the Spinner component with various styles.
//
        ...
        options.filter = { _, message in
            ...
            case let key as KeyMsg:
                ...
                case "q", "ctrl+c":
                    return QuitMsg()
                default:
                    return SpinnerModel.Message.key(key)
                }
            case let tick as SpinnerTickMsg:
                return SpinnerModel.Message.spinnerTick(tick)
            default:
                return message
            }
        }
