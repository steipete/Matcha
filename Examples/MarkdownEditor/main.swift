...
public struct MarkdownEditorModel: Model {
    ...
    var visualStart: (line: Int, column: Int)?
    ...
    enum EditorMode {
        ...
        var color: String {
            switch self {
            case .normal: "\u{1B}[34m" // Blue
            case .insert: "\u{1B}[32m" // Green
            case .visual: "\u{1B}[33m" // Yellow
            case .command: "\u{1B}[35m" // Magenta
            }
        }
    }
...
extension MarkdownEditorModel {
    ...
    func renderStatusLine() -> String {
        ...
        // Command line
        if case let .command(partial) = mode {
            return " :\(partial)█"
        }
...
func handleNormalMode(
    model: inout MarkdownEditorModel,
    key: KeyMsg
) -> (MarkdownEditorModel, Command<MarkdownEditorModel.Message>?) {
    switch key.type {
...
func handleInsertMode(
    ...
    case let .character(char):
        ...
        model
            .content[model.cursorLine] = String(line.prefix(upTo: index)) + String(char) +
            String(line.suffix(from: index))
        model.cursorColumn += 1
        model.isDirty = true
