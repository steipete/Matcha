...
//  MatchaBubbles
//
//  A tab navigation component for Matcha TUI applications.
//
...
public struct Tabs: Sendable {
    ...
    public func view() -> String {
        ...
        let tabWidths: [Int]
        if equalWidth {
            let maxWidth = items.map { $0.count }.max() ?? 0
        ...
        for (index, item) in items.enumerated() {
            ...
            if equalWidth {
                ...
                if currentWidth < targetWidth {
                    ...
                    finalText = String(repeating: " ", count: leftExtra) + paddedText + String(repeating: " ", count: rightExtra)
                } else {
