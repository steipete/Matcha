...
//  MatchaTests
//
//  Performance baseline tests to ensure Matcha meets performance requirements.
//
...
struct PerformanceBaselineTests {
    ...
    func testModelUpdateBaseline() throws {
    ...
    func testViewRenderingBaseline() throws {
    ...
    func testInputParsingBaseline() throws {
    ...
    func testRenderingFPSBaseline() async throws {
    ...
    func testLinearScalability() throws {
    ...
    func testFileBrowserPerformance() throws {
    ...
    func testTextEditorResponsiveness() throws {
...
struct PerformanceReportTests {
    ...
    func testGeneratePerformanceReport() async throws {
        ...
        print("Average execution time: \(formatTime(results.map { $0.averageTime }.reduce(0, +) / Double(results.count)))")
        print("Total memory allocated: \(formatMemory(results.map { $0.memoryAfter - $0.memoryBefore }.reduce(0, +)))")
        
        // Find outliers
        if !results.isEmpty {
            let slowestBenchmark = results.max(by: { $0.averageTime < $1.averageTime })!
            let mostMemory = results.max(by: { ($0.memoryAfter - $0.memoryBefore) < ($1.memoryAfter - $1.memoryBefore) })!
