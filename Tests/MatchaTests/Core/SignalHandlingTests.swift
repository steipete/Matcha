...
struct SignalHandlingTests {
    ...
    @MainActor
    func windowSizeSignal() async throws {
    ...
    @MainActor
    func signalsDisabled() async throws {
        let model = TestModel()
        var options = ProgramOptions.default
        options.output = TestOutputStream()
    ...
    @Test("Custom signal handler option")
    @MainActor
    func withoutSignalHandler() async throws {
        let model = TestModel()
        var options = ProgramOptions.default
        options.output = TestOutputStream()
    ...
    @Test("Multiple rapid quit signals")
    @MainActor
    func multipleQuitSignals() async throws {
        let model = TestModel()
        var options = ProgramOptions.default
        options.output = TestOutputStream()
    ...
    @Test("Signal handling during command execution")
    @MainActor
    func signalDuringCommand() async throws {
        struct LongRunningModel: Model {
            ...
            func view() -> String {
                "Command: \(commandStarted ? "started" : "waiting") - \(commandCompleted ? "completed" : "running")"
            }
        }
    ...
    @MainActor
    func rawModeWithSignals() async throws {
        let model = TestModel()
        var options = ProgramOptions.default
        options.output = TestOutputStream()
        options.handleSignals = true
    ...
    func focusEvents() async throws {
        struct FocusModel: Model {
            ...
            func view() -> String {
                "Focus: \(hasFocus ? "YES" : "NO") (changes: \(focusChangeCount))"
            }
        }
