...
public class ProgramTester<M: Model> {
    ...
    public func sendKey(_ key: String) async {
        let keyMsg = if key.count == 1 {
            KeyMsg(character: Character(key))
        } else {
            // Handle special keys
            switch key {
            case "enter": KeyMsg(type: .enter)
            case "escape": KeyMsg(type: .escape)
            case "up": KeyMsg(type: .up)
            case "down": KeyMsg(type: .down)
            case "left": KeyMsg(type: .left)
            case "right": KeyMsg(type: .right)
            default: KeyMsg(character: "?")
            }
        }
    ...
    public func expectView(equalTo expected: String, sourceLocation: SourceLocation = #_sourceLocation) async throws {
        ...
        #expect(
            view.trimmingCharacters(in: .whitespacesAndNewlines) == expected
...
public func mockKey(_ key: String, alt: Bool = false) -> KeyMsg {
    if key.count == 1 {
        KeyMsg(character: Character(key), alt: alt)
    } else {
        // Handle special keys
        switch key {
        case "enter": KeyMsg(type: .enter, alt: alt)
        case "escape": KeyMsg(type: .escape, alt: alt)
        case "up": KeyMsg(type: .up, alt: alt)
        case "down": KeyMsg(type: .down, alt: alt)
        case "left": KeyMsg(type: .left, alt: alt)
        case "right": KeyMsg(type: .right, alt: alt)
        default: KeyMsg(character: "?", alt: alt)
        }
    }
}
...
public func mockWindowSize(width: Int, height: Int) -> WindowSizeMsg {
    WindowSizeMsg(width: width, height: height)
}

/// Asserts that a closure throws a specific error type
public func assertThrows<E: Error>(
